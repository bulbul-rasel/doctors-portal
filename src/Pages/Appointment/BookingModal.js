...
const BookingModal = ({ date, treatment, setTreatment, refetch }) => {
    ...
    const handleBooking = event => {
        ...
        fetch('https://frozen-meadow-83396.herokuapp.com/booking', {
            method: 'POST',
            headers: {
            ...
            .then(data => {
                if (data.success) {
                    toast.success(`Appointment is Booked: ${formattedDate} at ${slot}`)
                } else {
                    toast.error(`You already booked this Appointment on ${data.booking?.date} at ${data.booking?.slot}`)
                }
                refetch()
                //close to modal
                setTreatment(null)
            ...
            <div className="modal modal-bottom sm:modal-middle">
                <div className="modal-box">
                    ...
                    <form onSubmit={handleBooking} className='grid grid-cols-1 mt-3 gap-2 justify-items-center'>
                        ...
                        <input type="text" name='name' disabled value={user?.displayName || ''} className="input input-ghost w-full max-w-xs" />
                        <input type="email" name='email' disabled value={user?.email || ''} className="input input-ghost w-full max-w-xs" />
